    ...
    if st.button("Calcular HCl"):
        for i  in range(len (df2)):
            ...
            if ct1>0:
                ...
                df3.at[i,"%_ERROR"]=round(error2,4)
            else:
                df3.at[i,"%_ERROR"]=0.0
    ...
    if prom_cHCl:
        ...
        if ct1>0:
          ...
          with colexp:
              ...
              V_NaOHexp=st.number_input("V. NaOH usado (ml): ",min_value=0.0, max_value=10000)
              V_HClexp=st.number_input("V. HCl usado (ml): ",min_value=0.0,max_value=10000)
