...
# 1. Agregamos la carpeta PYTHON al buscador de módulos
# Esto es vital para que tus apps encuentren la carpeta Handbook
ruta_python = os.path.join(os.path.dirname(__file__), "PYTHON")
if ruta_python not in sys.path:
    sys.path.insert(0, ruta_python)

# 2. Definimos las páginas (Quitamos el import * que daba error)
